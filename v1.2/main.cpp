...
_l_choosefile_proc_fileextcheck_complete:
	...
	{
		case PB_I16_1CH:
			p_audio = new AudioPB_i16_1ch(&pb_params);
			break;

		case PB_I16_2CH:
			p_audio = new AudioPB_i16_2ch(&pb_params);
			break;

		case PB_I24_1CH:
			p_audio = new AudioPB_i24_1ch(&pb_params);
			break;

		case PB_I24_2CH:
			p_audio = new AudioPB_i24_2ch(&pb_params);
			break;
	}
...
INT WINAPI filein_get_params(VOID)
{
	...
	while(TRUE)
	{
		...
		u32 = *((UINT32*) (p_headerinfo + bytepos + 4u));

		bytepos += (SIZE_T) (u32 + 8u);
	}

	u16 = *((UINT16*) (p_headerinfo + bytepos + 8u));
	...
	n_channels = *((UINT16*) (p_headerinfo + bytepos + 10u));

	pb_params.sample_rate = *((UINT32*) (p_headerinfo + bytepos + 12u));

	bit_depth = *((UINT16*) (p_headerinfo + bytepos + 22u));
