...
INT WINAPI file_get_params(VOID)
{
	...
	while(TRUE)
	{
		...
		if(compare_signature("fmt ", header_info, bytepos)) break;
	...
	while(TRUE)
	{
		...
		if(compare_signature("data", header_info, bytepos)) break;
...
_l_file_get_params_error:
	if(header_info != NULL) HeapFree(p_processheap, 0u, header_info);
	return -1;
}
